...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    keywords: category
      ? `${service.title}, ${category.name}, ${category.services.join(", ")}`
      : service.title,
  };
}
        ...
        <div className="bg-white py-16">
          <div className="!max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-[#cd1f38] mb-4">
              ...
              {category.services
                ...
                .map((relatedServiceSlug) => (
                  <a
                    ...
                    className="group bg-white border-2 border-gray-100 rounded-xl p-6 hover:border-[#cd1f38]/30 hover:shadow-lg transition-all duration-300"
                  >
                    <div className="flex items-center mb-3">
                      <span className="text-2xl">
                        {category.heroContent.icon}
                      </span>
                      <h3 className="font-semibold text-gray-900 group-hover:text-[#cd1f38] transition-colors capitalize">
                        {relatedServiceSlug.replace(/-/g, " ")}
                      </h3>
                    </div>
                    <p className="text-gray-600 text-sm">
                      Professional {relatedServiceSlug.replace(/-/g, " ")}{" "}
                      services to help your business grow.
                    </p>
                    <div className="mt-4 text-[#cd1f38] text-sm font-medium group-hover:text-[#7a0a20]">
                      Learn more →
                    </div>
                  </a>
                ))}
